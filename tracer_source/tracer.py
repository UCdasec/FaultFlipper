...
from capstone import (
    Cs,
    CS_ARCH_X86, CS_MODE_64, CS_ARCH_ARM,
...
def parse_line_numbers(line_specs: list[str]) -> list[int]:
    """Helper for CLI to parse input line numbers.
        ...
        for cu in dwarfinfo.iter_CUs():
            ...
            for entry in line_program.get_entries():
                ...
                if state.file == 0 or state.line == 0:
                    print(f"state line is 0 (or file is 0)")
                    continue
...
def main():
    ...
    if args.critical_asm is not None:
        ...
        for i, (k, v) in enumerate(addr_to_line.items()):
            ...
            print(f"[SOURCE] {v} maps to address range: [ASM] 0x{k:x} (exclusively) to 0x{next_key:x}")
