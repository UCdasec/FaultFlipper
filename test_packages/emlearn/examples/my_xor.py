...
def predict(bin_path, X, verbose=1, arm=False):
...
def evaluate_model(bin_path:Path, test, arm):

    # Make predictions on dataset
    X_test = test[['x','y']]
    Y_test = test[['label']]

    y_pred_c = predict(str(bin_path), X_test, arm=arm)
...
if __name__ == "__main__":
    ...
    #compiled_model_path = compile_model(Path('examples'), Path('xor.c'))
    if IS_ARM:
        model = Path("tree_classifier_arm32.o").absolute()
    else:
        model = Path("tree_classifier_x86.o").absolute()

    acc =evaluate_model(model, test, arm=IS_ARM)
    print(f"{acc}")
